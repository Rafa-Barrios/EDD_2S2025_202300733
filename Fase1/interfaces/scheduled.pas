...
begin
  ...
  // Intentar enviar correo al inbox del destinatario
  if AddMailToJson('data/inbox.json', nextMail^.sender, nextMail^.recipient, nextMail^.subject, nextMail^.message) then
  begin
    // Eliminar correo de scheduled.json
    RemoveScheduledMailFromJson('data/scheduled.json', nextMail^.id);
...
begin
  ...
  LoadScheduledFromJson('data/scheduled.json', current_user_email);
