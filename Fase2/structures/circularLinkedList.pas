...
function CL_RemoveFromList(var head: PCNode; const email: string): Boolean;
...
// Elimina un contacto por email de la lista circular
function CL_RemoveFromList(var head: PCNode; const email: string): Boolean;
var
  current, prev: PCNode;
begin
  ...
  repeat
    if SameText(current^.email, email) then
    begin
      if prev = nil then
      begin
        // Eliminando el primer nodo
        if current^.siguiente = current then
          head := nil
        else
        begin
          head := current^.siguiente;
          prev := head;
          while prev^.siguiente <> current do
            prev := prev^.siguiente;
          prev^.siguiente := head;
        end;
      end
