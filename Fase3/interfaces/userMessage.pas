...
uses
  gtk2, glib2, SysUtils,
  doubleLinkedList, userInbox, jsonTools, variables; // ✅ importamos variables para rutas y email actual
...
var
  ...
  btnClose, btnDelete, btnFavorite: PGtkWidget;
...
begin
  ...
  begin
    // ✅ Movemos el correo a la papelera
    if MoveMailToTrash(variables.json_file_mails, variables.json_file_trash,
    ...
    // cerramos ventana y recargamos bandeja
    gtk_widget_destroy(msgWindow);
    ReloadInbox;
  end;
end;
...
begin
  if currentMail <> nil then
  begin
    // ✅ Movemos el correo a favoritos
    if MoveMailToTrash(variables.json_file_mails, variables.json_file_favorites,
                        current_user_email, currentMail^.subject) then
    begin
...
begin
  ...
  currentMail := mail; // referencia para usar en eliminar/favoritos

  // Si estaba como "No leído", marcarlo como leído
  ...
  // =============================
  // Botones (Cerrar, Eliminar, Favoritos)
  // =============================
